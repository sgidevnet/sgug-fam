...
int FAMOpen2(FAMConnection* fc, const char* appName)
{
    ...
    if (appName) {
	char msg[200];
	snprintf(msg, sizeof(msg), "N0 %ld %ld %s\n", geteuid(), getegid(), appName);
	       ...
	       void* userData, int code)
{
    ...
    char msg[MSGBUFSIZ];
    int msgLen;
    snprintf(msg, MSGBUFSIZ, "%c%d %ld %ld %s\n", code, reqnum, geteuid(),
		     ...
		     const char* mask)
{
    ...
    char msg[MSGBUFSIZ];
    int msgLen;
    snprintf(msg, MSGBUFSIZ, "F%d %ld %ld %s\n", fr->reqnum, geteuid(),
                             groups.groups[0], filename);
    msgLen = strlen(msg) + 1;  // include terminating \0 in msg
...
sendSimpleMessage(char code, FAMConnection *fc, const FAMRequest *fr)
{
    // Create FAM String
    char msg[MSGBUFSIZ];
    snprintf(msg, MSGBUFSIZ, "%c%d %ld %ld\n", code, fr->reqnum, geteuid(), getegid());
...
GroupStuff::groupString(char *buf, int buflen)
{
    ...
    {
        snprintf(p, GROUP_STRLEN, " %ld", groups[i]);
        p += strlen(p);
    }
    *p = '\0';
